import json
import numpy as np
from tqdm import tqdm
import matplotlib.pyplot as plt
from scipy.stats import pearsonr

def InvSbox(data):
    inv_s = [
        
    ]
    return inv_s[data]

# intから2進数文字列を生成 ex. 127 -> 01111111
def Int2BinStr(data):
    return format(data, "08b")

def HamDistance(s1, s2):
    return sum(c1 != c2 for c1, c2 in zip(Int2BinStr(s1), Int2BinStr(s2)))

def HamWeight(s):
    return sum(c != '0' for c in Int2BinStr(s))

partialKeys = [x for x in range(256)]
sample = 100000
numPoint = 10000
avgCount = 0

# load cipher texts
print('---Loading Cipher Texts---')
path = 'cipher.txt'
cipherTexts = list()
with open(path) as f:
    for _ in tqdm(range(sample)):
        cipherTexts.append(list(map(int, f.readline().split())))

cipherTexts = np.array(cipherTexts, dtype=np.uint8)

# load power traces
print('---Loading Power Traces---')
filename = 'WaveFormWithoutC'
trace = list()
with open(filename) as f:
    for _ in tqdm(range(sample)):
        trace.append(list(map(int, f.readline().split())))

T = np.array(trace, dtype=np.int16)

del filename, _

# 中間値配列の生成
print('---Generating Intermediate Values---')
V = np.array([[InvSbox(cipherTexts[i][0] ^ partialKeys[j]) for j in range(256)] for i in tqdm(range(sample))])

# 中間値からハミング重みモデルへ
#H = np.array([[HamWeight(cipherTexts[i][0] ^ x) for x in V[i]] for i in range(sample)])

# 中間値からハミング距離モデルへ
print('---Mapping Intermediate Values to Hamming Distance Model---')
H = np.array([[HamDistance(x, cipherTexts[i][0]) for x in V[i]] for i in tqdm(range(sample))])
del V

print('---Calculating Correlation Coefficient---')
r = np.zeros([256, numPoint], dtype=np.float64)
for i in tqdm(range(256)):
    for j in range(numPoint):
        r[i][j] = pearsonr(H.T[i], T.T[j])[0]

print('---Plotting Correlation Coefficient---')
for i in tqdm(range(256)):
    plt.plot(r[i])
    plt.ylim(r.min(), r.max())
    plt.ylabel('Correlation', fontsize=20)
    plt.xlabel('Time', fontsize=20)
    plt.savefig('{}avg/DPA_{}sample/{:02X}.png'.format(avgCount, sample, i))
    plt.clf()

print('{:02X}'.format(np.argmax([np.amax(list(map(abs, r[i]))) for i in range(256)])))
